...
呈现器消费解析后的主题快照，并投影到 document：`html { color-scheme }` 驱动原生 UA 控件，依据当前配色方案设置 `body[data-ds-dark-theme]`，把主题的别名 token 与 `--dsh-content-font-size` 设为 body 上的内联变量（在粗指针设备上该轴发布为 `max(16px, <偏好>px)`，因此手机字号不会低于 16px，内容字号阶梯按该尺寸重排，而不是依赖浏览器会裁切页面的聚焦缩放），并持有一个 `<meta name="theme-color">`，其内容随计算后的 body 背景色更新。对呈现器执行 dispose（资源释放）时，它会连同其他全局写入一起移除自己的元数据节点。
...
- **768px 到 1024px 之间的窄窗口**——右侧面板关闭后，中栏仍可能小于 400px；左侧 56px 控制栏仍会保留。低于 768px 时控制栏消失，但手机模式的打开控件覆盖中栏左上角，在该处绘制自有前导控件的栏占用方必须读取 `--dsh-frame-leading-inset`。
