...
选择器在主列表中显示标准模式和 Beardy 模式，并将 PTC、极简和创造模式放入可折叠的**更多模式**分组。Beardy Discord 和 Beardy 无人值守模式默认隐藏。只有部署提供且健康的 preset 才会出现；未指定显示位置的自定义 preset 保留在主列表中。无论显示位置如何，当前选中的模式始终位于主列表；chip 挂载期间会记住分组的展开状态。

选择**管理模式…**，即可将任意健康模式移至**主列表**、**更多模式**或**隐藏**。显示位置通过宿主设置提供方持久保存，保存成功后生效；保存失败时，对话框会显示错误。隐藏只影响选择器：集成仍可选择该 preset，现有会话标题保留其名称，设置页面保留完整名单。
