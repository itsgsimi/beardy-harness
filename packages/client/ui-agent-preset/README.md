...
The picker shows Standard and Beardy in its main list and places PTC, Minimal, and Creator under the collapsible **More modes** group. Beardy Discord and Beardy unattended are hidden by default. Only healthy presets supplied by the deployment appear; custom presets without a placement stay in the main list. The selected mode remains in the main list regardless of placement, and the group remembers its expansion while the chip remains mounted.

Choose **Manage modes…** to move any healthy mode between **Main list**, **More modes**, and **Hidden**. Placements persist through the host settings provider and apply after the save succeeds; a failed save displays its error in the dialog. Hiding changes only the picker: integrations can still select the preset, existing session headers keep its name, and Settings retains the full roster.
