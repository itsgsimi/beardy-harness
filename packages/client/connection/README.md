...
Before authentication, every API request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401 when authentication is enabled. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).

Setting `insecureNoAuth: true` on the Connection row explicitly disables browser authentication; the default is `false`. Startup URLs contain no token, and Connection does not load or create the browser signing record. Index requests also pass the Host/Origin checks in this mode. Every accepted caller can invoke all Host operations, including command execution. Configure network binding through the [Web bundle](../../bundle/web-app/README.md).
...
**Runtime invariant:** No companion is published. Browser-session verification uses the signing secret loaded during activation, while the credentials companion owns record commit-event lifetime. Stream/reconnect sequencing and rpcId round-trip discipline are exercised directly by behavior specs, and route register/dispose symmetry is audited by the webserver companion.
