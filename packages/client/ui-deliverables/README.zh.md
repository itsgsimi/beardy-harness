...
本包渲染已完成轮次末尾的产出文件行——列出修改工具创建或修改的文件——并把收尾正文中匹配的行内代码引用转为链接，让被点名的文件在右侧 Sidebar 中打开。它还在聊天中直接展示视觉发现——图表、截图、GIF、SVG 示意图和交互式 HTML 原型，附带说明、放大预览及原始文件下载；源文件改变或聊天重新加载后，保存的视觉内容仍然可用。链接路径来自成功的文件修改与显式交付，而非收尾正文——无论模型是否记得点名，产出文件都会被列出。正式提供的组合中只有 Web patch 加载本包；删除其 cordis.yml 条目会同时移除指引、文件行、正文链接与视觉发现。
...
`present_visual` 在智能体发布内容的位置添加视觉卡片，代码执行中的调用同样适用。卡片在发现说明旁展示图表或截图，保留 GIF 动画和 SVG 矢量，并提供放大查看及原始文件下载。即使源文件改变或消失，重新加载聊天仍保留捕获的字节。HTML 原型在不透明来源的沙箱 iframe 中运行；内联脚本和样式可用，对父页面、外部子资源、表单、弹窗和嵌套框架的访问受到限制。放大 HTML 原型会创建独立实例。

模型指引要求在有帮助时用图表说明数据、用截图报告浏览器发现、用 GIF 展示交互、用示意图解释结构，并用自包含 HTML 展示交互图表或界面提案。指引要求配套文本、标明示例数据，并区分原型和实测截图。普通文件交付仍使用 `present`。
...
固定段落要求模型以可点击的行内代码引用主要文件，并在工具可用且有帮助时主动使用 `present_visual`。指引把各类视觉内容与适合的解释用途关联，并要求以文本提供发现说明。
