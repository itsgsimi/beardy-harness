...
在 Web 聊天中直接展示文件交付与视觉发现。智能体可发布图表、截图、GIF、SVG 示意图和交互式 HTML 原型，附带说明、放大预览及原始文件下载。源文件改变或聊天重新加载后，保存的视觉内容仍然可用。文件卡片和匹配的引用打开当前工作区源文件，已完成轮次的文件行还会列出成功的编辑修改。移除本插件会移除这些展示和模型指引。
