...
Show file deliveries and visual findings directly in Web chat. Agents can publish charts, screenshots, GIFs, SVG diagrams, and interactive HTML mockups with explanatory captions, expanded previews, and original downloads. Saved visuals survive source-file changes and reloads. File cards and matching references open current workspace sources, while the completed-turn file row also lists successful editor changes. Removing this plugin removes its presentation and model guidance.
...
`present_visual` adds a visual card at the point where the agent publishes it, including during code execution. Cards show charts or screenshots alongside findings, preserve GIF animation and SVG vectors, and offer an expanded view and original-file download. Reloading the chat retains the captured bytes even when the source changes or disappears. HTML mockups run in an opaque sandboxed iframe; inline scripts and styles work, while parent-page access, external subresources, forms, popups, and nested frames are restricted. Expanding an HTML mockup creates a separate instance of it.

The model guidance asks agents to use charts with data explanations, screenshots with browser findings, GIFs for interaction, diagrams for structure, and self-contained HTML for interactive charts or UI proposals when useful. It requires accompanying text and labels for illustrative data, and distinguishes mockups from observed screenshots. Ordinary file delivery still uses `present`.
...
One fixed paragraph instructs the model to name primary files as clickable inline-code references and to use `present_visual` proactively when available and useful. The guidance connects each supported visual form to its appropriate explanatory use and requires supporting findings in text.
...
- **Ordinary file declarations do not preserve contents** — reopening or transferring a Session requires source files accessible through the viewed Session’s filesystem. Missing files, directories, and final symbolic links return 404.
