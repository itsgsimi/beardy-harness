...
`MenuItem.expanded` exposes an inline disclosure's open state; `onSelect` lets the owner toggle it and supply the visible rows. Keep the disclosure's `id` stable to retain focus while its following rows change. Portaled menus reposition when their list dimensions change, including expansion near a viewport edge.
