...
- `websocketHeartbeatIntervalMs` is both the Ping cadence and the Pong deadline. The Host terminates a peer that does not answer before the next interval, so a deployment whose event loop or network can stall longer than this interval must raise it. The Client's silent-socket deadline is three times this interval, so raising it also lengthens how long a suspended browser socket goes unnoticed.
