...
History pages and follow opening snapshots carry one `{ type: 'event', event: SessionWireEvent }` record per durable Session event. One page is bounded twice: by its message count, 50 by default, and by roughly one million serialized characters across its events. The second bound exists because a page carries every event between its messages, and one turn's tool results can outweigh the messages around them, so a long Session opened at the default count would otherwise serialize into a single multi-megabyte frame written ahead of that follow's Assistant frames. Whichever bound trips first ends the page at a message boundary and reports `hasMore`; the newest message always ships with its whole group, even when that group alone exceeds the size bound. The Client retains each accepted record as one durable `SessionEventLikeEntry`; Assistant token boundaries remain inside the compact stream on `assistant/message` or `assistant/attempt`. Tool arguments, result content, failures, and `tool/result.data.meta` pass through unchanged; the controller does not resolve a Tool definition, run a presenter, or attach UI data.
