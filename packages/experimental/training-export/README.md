...
For every `llm/stream` call whose `sessionId` is set, `<root>/<session-id-escaped>/samples.jsonl` gains one line after the stream settles — normally, on abort, or on a downstream throw — carrying the folded response and, on failure, a `{name, message, code?}` error. `<root>/<session-id-escaped>/meta.json` is written once, at the first sample, with the session's `agentPreset` (`SessionHeader.agentPreset`, or `null`) and its latest known `title` (`null` until a `session/title` event arrives); a title landing after the first sample re-writes the whole file, off the same per-session queue. `<root>/<session-id-escaped>/labels.jsonl` gains one line per `turn/end`, with `startedAt`/`durationMs` (from the turn's `turn/start`/`turn/end` event times), the turn's sample `seq`s, tool/hook/approval counts, `toolCallOutcomes` (one entry per `tool/call`, matched to its `tool/result` by `callId`, with `isError`/`durationMs`/`resultChars` — `null`/`null`/`0` for a call still unmatched at turn end — and the `seq` of the sample whose response carried that call), `assistantChars` (total UTF-8 length of the turn's assistant `text` blocks, excluding reasoning and tool-call arguments), a `git diff --shortstat` between a full working-tree snapshot taken at the turn's first allow-listed-provider sample and a second one taken at `turn/end` — so `diff` reflects only what this turn changed, not a pre-existing dirty tree — and any ratings recorded through `dsh-message-feedback` for that turn's assistant messages. A request with no `sessionId`, or one naming a session the store no longer holds, passes through completely untouched — this plugin only calls `next()` in that case, exactly like every other request.
...
The plugin wraps the `llm/stream` waterfall the same way `session-checkpoint-policy` does — `next()` is delegated to unconditionally when there is no live session, and wrapped exactly once otherwise. The wrapper folds `block-end` chunks into `response.content` in index order, keeps the last `usage` chunk, and records `finish` verbatim; a terminal `error`/`aborted` finish reason or a thrown rejection both populate `response.error`, and a thrown rejection is rethrown after being recorded — the sample is written either way. That same fold also correlates each `tool-call` content block's id to the sample's own `seq` and adds each `text` block's UTF-8 length to the turn's running `assistantChars`, both kept on the open turn's aggregate for the eventual label. A separate `session/event` listener tracks turn/step position and the current turn's running counts (tool calls/errors and, per call, its request/result timing and result size; hook outcomes; approval counts; compaction-in-turn; the assistant-message ids to look up ratings for; and the session's latest `session/title`, re-writing `meta.json` when one lands after the first sample), since `Session` exposes no public current-turn/step accessor. Both listeners share one `WeakMap<Session, …>` created inside `apply()`, so state is scoped to the plugin's own fiber and needs no ambient module-level state.
