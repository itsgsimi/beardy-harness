...
export default class ProviderCwdFileSystem extends FileSystem {
  ...
  override async makeDirectory(): Promise<never> { throw new FsError('Read-only provider fixture', 'FS_SANDBOX_DENIED') }
  override async removeFile(): Promise<never> { throw new FsError('Read-only provider fixture', 'FS_SANDBOX_DENIED') }
}
