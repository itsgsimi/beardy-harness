...
Run `dsh --profile web` to open an interactive browser GUI with chat, model and settings management, and session history. It uses the same model access, tools, and safety defaults as other dsh surfaces. Startup prints an authenticated URL and normally opens it in the default browser; SSH sessions and `--no-open` leave the URL for manual opening. The explicit `--insecure-no-auth` flag permits unauthenticated LAN access. Choose this package for interactive browser work; use `dsh-headless` for one-shot command-line tasks.
...
By default the GUI accepts connections from this machine only and authenticates browser pages, Host API methods, and WebSocket streams. To bind all network interfaces without authentication, explicitly enable the insecure mode; port `0` lets the OS allocate an available port:

```sh
dsh --profile web --no-open --port 0 --host 0.0.0.0 --insecure-no-auth
```

This invocation prints a warning and clean local and LAN URLs. Anyone who can reach the server can operate the harness without a token or cookie. Host and Origin checks remain active, and `--trusted-host` adds extra authorities; these checks do not authenticate callers. The flag applies only to that invocation. LAN addresses are sampled once at startup, so restart after a network change to re-advertise them. The [Connection package](../../client/connection/README.md) owns authentication and request trust.
...
| [`src/startup.ts`](src/startup.ts) | The `web-startup` provider: `--host`, `--port`, `--trusted-host`, `--no-open`, `--insecure-no-auth`, `--help` |
...
- **Binding all network interfaces requires explicit authentication bypass** — `--host 0.0.0.0` is rejected unless `--insecure-no-auth` is present.
