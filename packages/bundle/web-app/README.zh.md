...
运行 `dsh --profile web`，打开提供聊天、模型与设置管理以及会话历史的交互式浏览器 GUI。它使用与其他 dsh 表层相同的模型访问、工具与安全默认值。启动时会打印带认证信息的 URL，通常还会在默认浏览器中打开；SSH 会话和 `--no-open` 会保留该 URL，供你手动打开。显式传入 `--insecure-no-auth` flag 可启用无需认证的 LAN 访问。需要在浏览器中交互式工作时选择本包；一次性的命令行任务应使用 `dsh-headless`。
...
默认情况下，启动时会打印 `dsh web:` 行，其根 URL 携带新的进程 token。除非 `--no-open` 或 SSH 会话抑制，否则默认浏览器会打开该 URL、取得签名 cookie，再重定向到不含认证参数的根页面。页面加载且你可以与 agent 对话，就说明成功了。两种可预期的失败：前端未构建时，启动会以构建提示停止（checkout 中运行 `pnpm run build`）；浏览器无法打开时，stderr 会打印不含凭据的诊断，但服务器会继续运行——请自行打开已打印的启动 URL。
...
默认情况下 GUI 只接受本机的连接，并对浏览器页面、Host API 方法与 WebSocket 流进行认证。要在无需认证的情况下绑定所有网络接口，必须显式启用不安全模式；端口 `0` 让操作系统分配可用端口：

```sh
dsh --profile web --no-open --port 0 --host 0.0.0.0 --insecure-no-auth
```

本次调用会打印警告以及不含认证参数的本地与 LAN URL。任何能够连接服务器的人都可操作 harness，无需 token 或 cookie。Host 与 Origin 检查仍然有效，`--trusted-host` 可添加额外权威标识；这些检查不会认证调用方。该 flag 仅对本次调用生效。LAN 地址只在启动时采样一次，网络变化后请重启以重新公告。[Connection 包](../../client/connection/README.zh.md)负责认证与请求信任规则。
...
| [`src/startup.ts`](src/startup.ts) | `web-startup` 提供方：`--host`、`--port`、`--trusted-host`、`--no-open`、`--insecure-no-auth`、`--help` |
...
- **绑定所有网络接口需要显式绕过认证**——未传入 `--insecure-no-auth` 时，`--host 0.0.0.0` 会被拒绝。
