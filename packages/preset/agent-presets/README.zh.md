...
可选的 preset 来自三类来源：本包 `presets/` 下随包交付的 preset、已配置的根目录，以及你自己放在 `<dshHome>/.agent-presets` 下的 preset。每个 preset 都可发布显示名与描述。设置名单会列出组装无法加载的 preset 及其原因，因此你能看到该修什么或删什么。
...
preset 可在 `preset.yml` 中通过可选的 `picker` 字段选择展示位置：`main` 表示主列表，`more` 表示可折叠分组，`hidden` 表示从普通选择列表中隐藏。展示位置元数据缺失或无效时按 `main` 处理。标准模式与 Beardy 使用 `main`；PTC、极简模式与创造模式使用 `more`；Discord 与无人值守变体使用 `hidden`。展示位置只影响呈现：隐藏的 preset 仍保留在完整名单中，仍可组装会话、运行自动任务，并按 id 选择。

`agent-presets` settings 命名空间接受一个 `picker` 映射，以 preset id 为键，以上述值为值。用户条目覆盖对应 preset 的元数据；移除该条目后恢复其发布的展示位置。更新一个条目会与其他已保存的位置合并，并保留默认 preset。无效的设置值会被拒绝。
...
- **preset 文件是输入，绝不是持久化目标。** 被挂载的子树和本包的 include 行把 `write()` 覆写为空操作，因此 loader 发起的写回绝不会重写共享的 preset 文件。
- **include 与外层共享 scope。** 继承的能力名单只 include 一次；重复列出其中的行会造成重复注册。patch 可以到达被 include 文件的直接行与 group，但不会穿过另一个文件 include。Creator 和 Beardy 会先禁用继承的角色专属行，再提供其派生 preset 可继续 patch 的直接替代行。
...
复制会解引用符号链接以保证自包含，把目录树收紧为仅属主可用（文件 `0o600` 并保留属主执行位，目录 `0o700`），并在首次复制时创建根目录。复制出的 `preset.yml` 会被重写：保留来源的描述供作者编辑，丢弃其名称、roster `order` 与 `picker` 展示位置。副本拥有自己的身份，并在用户未覆盖时使用主列表位置。删除拒绝随部署提供的 preset，并清除指向刚删除 preset 的用户默认值。
