...
describe('the shipped preset root', () => {
  it('supplies the built-in presets from a bare roster, healthy and system-trusted', async () => {
    ...
    expect(listed.map(preset => preset.id).sort())
      .toEqual(['beardy', 'beardy-discord', 'beardy-unattended', 'cordis', 'minimal', 'ptc', 'standard'])
  ...
  it('enables web_fetch in presets that own the Web tool row', async () => {
    for (const id of ['ptc', 'standard']) {
