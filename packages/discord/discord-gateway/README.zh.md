...
启用可选的[语音提供程序](../../speech/speech-whisper/README.zh.md)后，已准入的语音消息和音频附件会先通过本地后端转写，再进入普通频道对话。下载前仍会检查发送者和频道限制。音频沿用现有频道队列并保留消息来源，作为用户文本处理，不作为网关命令或批准答复。缺少提供程序时会明确回复语音不可用。
...
每条被准入的 Discord 消息都以一条用户角色（user-role）消息到达，正文即消息内容，长度受 `maxInputChars` 约束；被防抖窗口合并的消息以一条按换行拼接的消息到达。该消息带有由各界面渲染成提示的来源信息：发送者的 Discord 用户 id、频道 id、消息 id，以及写明频道的摘要。附件以 JSON 编码的文件名、URL、媒体类型和字节数附加到正文，并明确标记为不可信元数据；仅含附件的消息也会启动一轮对话。模型收到的是文件引用，而不是文件内容。成员角色和频道内的其他消息不会进入模型，因此此前的对话内容来自本包已追加的会话历史。
...
- **仅传递附件引用** —— 网关不下载或解析文件，并忽略 embed；入站表情回应仅在该频道有审批等待时有效。过长的入站文本在 `maxInputChars` 处截断，不拆成多轮。
