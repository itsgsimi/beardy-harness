...
With the optional [speech provider](../../speech/speech-whisper/README.md), admitted voice notes and audio attachments are transcribed through the local backend before entering the normal channel conversation. Sender and channel restrictions apply before download. Audio follows the existing per-channel queue, retains its message source, and is treated as user text rather than a gateway command or approval answer. A missing provider produces a clear voice-unavailable reply.
...
- **Attachment references only** — the gateway does not download or interpret files, and embeds are ignored; inbound reactions matter only while an approval waits in that channel. Long inbound text is truncated at `maxInputChars` rather than split across turns.
