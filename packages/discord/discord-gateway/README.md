...
- **Cron run delivery.** Finished scheduled runs of `@deepseek-ai/dsh-cron` carry a delivery channel; the gateway posts the run's final text there, or a one-line outcome note when the run produced no text and cron's `deliverOutcomes` asks for one. Runs without a delivery channel are left alone.
...
- **Proactive delivery is best-effort text** — if the post of a settled proactive turn or a cron run delivery fails, that text is dropped rather than retried, and only the final assistant text travels; intermediate narration stays in the session.
