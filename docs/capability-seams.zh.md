...
| `ctx.speech` | `seam` | [`speech-whisper`](../packages/speech/speech-whisper) | [`speech-whisper`](../packages/speech/speech-whisper) | [`discord-gateway`](../packages/discord/discord-gateway) | - | 经过大小限制的编码音频通过受管子进程服务解码，并发送到所配置的 whisper.cpp 端点；消费方通过普通会话输入提交识别出的文本。 |
