...
[显式无令牌模式](../feature/2026-09-09-explicit-token-free-web-access.zh.md)提供退出认证的方式，仅在带有 `--insecure-no-auth` 时允许 `--host 0.0.0.0`。默认 CLI 仍拒绝该绑定。浏览器认证不增加 TLS、转发 header 解释或代理配置。
