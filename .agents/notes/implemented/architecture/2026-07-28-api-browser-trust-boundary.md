...
- A non-loopback composition must trust its serving authorities or requests are refused, then satisfy the configured authentication policy like every loopback request. The shipped CLI permits `--host 0.0.0.0` only with `--insecure-no-auth`; `--trusted-host` only extends the Host/Origin fence and grants no identity.
- Clients must label POST bodies `application/json` (ours always did; raw-fetch tests gained the header).
- Host and Origin remain request-routing evidence only. When authentication is enabled, the process token and signed cookie establish the browser identity used by every Host method.
