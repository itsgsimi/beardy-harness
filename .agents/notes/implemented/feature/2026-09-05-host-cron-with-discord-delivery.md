...
Channel-to-Session continuity lives in memory, so a restart starts each Discord channel on a new Session. Guild channels deliver empty bodies until Message Content intent is enabled for the application in the Discord developer portal; direct messages arrive complete without it. Exactly one process may identify with a bot token, or every inbound message gets answered twice.
