...
Users can start the shipped agent with `dsh --profile beardy` or add the bundle to another profile. Beardy retains the standard agent's tools and prompt capabilities, can inspect and temporarily extend its live Cordis runtime, can search persisted session history and the configured SearXNG instance, and can fetch Web pages, while all model-visible inputs continue to be produced by existing logged capabilities. Durable harness or user-preset changes still use the shell and filesystem tools and follow the composition-authoring skill's ownership rules. `$DSH_HOME/SOUL.md` supplies editable personality guidance for Beardy sessions without changing other profiles. Curated personal memory arrives through [`dsh-tool-memory`](../../../../packages/memory/tool-memory/README.md), and token-gated scheduling and Discord delivery compose from the preset family in [Beardy composition layers](2026-09-05-beardy-composition-layers.md); an automatic skill curator remains a distinct capability rather than implicit Beardy behavior.
