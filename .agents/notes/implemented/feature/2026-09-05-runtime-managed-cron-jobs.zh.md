...
由 cron 直接发往 Discord 被否决了：投递属于拥有那个渠道的一方。类型化事件让 cron 保持平台中立，并为 Web 或其他监听者留出空间而不必改动本包。
...
网关的持久投递队列只发送最终 assistant 文本或已启用的结果通知。对已暂停任务执行 `run_now` 会以专门的提示被拒绝，而不是悄悄把它重新武装。会话仍然会累积——删除任务后其历史运行的会话仍留在磁盘上，与之前相同。
