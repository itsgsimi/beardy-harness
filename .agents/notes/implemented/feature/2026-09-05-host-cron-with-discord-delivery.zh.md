...
频道到会话的连续性存在于内存中，因此重启后每个 Discord 频道会从新会话开始。在开发者后台为该应用启用 Message Content intent 之前，服务器频道送达的消息正文为空；私信不需要它即可完整送达。恰好只能有一个进程用该 bot token identify，否则每条入站消息都会被回复两次。
