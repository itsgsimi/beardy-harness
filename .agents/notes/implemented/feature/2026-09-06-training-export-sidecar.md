...
Workspace capture (`git rev-parse HEAD` + `git status --porcelain`, `execFile`, 5s timeout, never throws) runs at most once per turn: the first sample whose provider is in the configured `providers` allow-list captures it, and every later sample that turn reuses the snapshot; a session with no `cwd` skips it permanently. That same capture also snapshots the full working tree (tracked + untracked, respecting `.gitignore`) as a tree object — `git add -A` then `git write-tree` against a throwaway `GIT_INDEX_FILE`, so the real index is never touched — keeping only the resulting hash. At `turn/end`, a second snapshot is taken the same way and `git diff --shortstat` runs between the two tree hashes to build the label's `diff`; diffing snapshot-to-snapshot instead of against the recorded `HEAD` was a fix (2026-09-06) for a defect where a tree already dirty before the turn started repeated in every later turn's `diff`. `providers` gates only these `git` reads — every provider's samples are written regardless, matching the halorun-side reader owning the provider filter.
...
### Additive fields: agentPreset, title, tool-call outcomes, turn timing

`meta.json` gained `agentPreset` (`session.header.agentPreset ?? null` — the same header already read for `parentSessionId`/`delegationDepth`/`cwd`/`createdAt`) and `title` (the latest `session/title` event's text, `null` until one arrives). Because `meta.json` is otherwise written once at the first sample, a `session/title` landing afterward re-writes the whole file from a shared `buildMeta()` — the same builder used for the first write — off the same per-session queue, never the hot path; `session/title` reaches this plugin's `SessionEventMap` switch the same way `hook/result`/`approval/*`/`compaction/*` already do, via a type-only `import type {} from '@deepseek-ai/dsh-session-title'` (added to `peerDependencies`/`devDependencies` and the matching `tsconfig.json` project reference). `labels.jsonl` gained `startedAt`/`durationMs` (the turn's own `turn/start`/`turn/end` event times), `assistantChars` (summed while folding each sample's response, alongside the existing per-turn aggregate), and `toolCallOutcomes` — one entry per `tool/call`, tracked from call to its matching `tool/result` by `callId` in the same open-turn aggregate that already counts `toolCalls`/`toolErrors`, plus a `callId → seq` map populated while folding each sample's response content, so a label can say which sample's response actually requested a given call.
